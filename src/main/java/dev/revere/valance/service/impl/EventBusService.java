...
public class EventBusService implements IEventBusService {
    ...
    public void setup(ClientContext context) throws ServiceException {
        ...
        if (this.eventBus == null) {
            ...
            System.out.println(LOG_PREFIX + "EventBus instance created.");
        } else {
            System.out.println(LOG_PREFIX + "[WARN] EventBus instance already exists during setup.");
        }
    ...
    public void register(Listener listener) {
        if (eventBus == null) {
            System.err.println(LOG_PREFIX + "[ERROR] Cannot register listener, EventBus is not available!");
            return;
        }
    ...
    public void unregister(Listener listener) {
        if (eventBus == null) {
            System.err.println(LOG_PREFIX + "[WARN] Cannot unregister listener, EventBus is not available (likely during shutdown).");
            return;
        }
    ...
    public <T extends IEvent> T post(T event) {
        if (eventBus == null) {
            System.err.println(LOG_PREFIX + "[ERROR] Cannot post event, EventBus is not available!");
            return event;
        }
