...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            ...
            try {
                event = new PacketEvent(PacketEvent.EventState.RECEIVING, p_channelRead0_2_);

                Optional<IEventBusService> serviceOpt = ClientLoader.getService(IEventBusService.class);
                if (serviceOpt.isPresent()) {
                    serviceOpt.get().post(event);
                    p_channelRead0_2_ = event.getRawPacket();
                }
            } catch (Throwable t) {
                System.err.println("[" + ClientLoader.CLIENT_NAME + ":Hook] Error posting PacketEvent (RECEIVING):");
                t.printStackTrace();
            }
    ...
    private void dispatchPacket(final Packet inPacket, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        PacketEvent event = null;
        Packet packetToSend = inPacket;

        try {
            event = new PacketEvent(PacketEvent.EventState.SENDING, packetToSend);

            Optional<IEventBusService> serviceOpt = ClientLoader.getService(IEventBusService.class);
            if (serviceOpt.isPresent()) {
                serviceOpt.get().post(event);
                packetToSend = event.getRawPacket();
            }
        } catch (Throwable t) {
            System.err.println("[" + ClientLoader.CLIENT_NAME + ":Hook] Error posting PacketEvent (SENDING):");
            t.printStackTrace();
        }
        ...
        final EnumConnectionState enumconnectionstate = EnumConnectionState.getFromPacket(packetToSend);
        ...
        if (this.channel.eventLoop().inEventLoop()) {
            ...
            ChannelFuture channelfuture = this.channel.writeAndFlush(packetToSend);
            ...
            final Packet finalPacketToSend = packetToSend;
            final EnumConnectionState finalEnumConnectionState = enumconnectionstate;

            this.channel.eventLoop().execute(new Runnable() {
                public void run() {
                    if (finalEnumConnectionState != enumconnectionstate1)
                    {
                        NetworkManager.this.setConnectionState(finalEnumConnectionState);
                    }
                    ChannelFuture channelfuture1 = NetworkManager.this.channel.writeAndFlush(finalPacketToSend);
